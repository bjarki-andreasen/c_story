...
LISTENER_ARG_LISTENERS_DEFINE(bar_set_listeners, BAR_N_LISTENERS);
...
{
	listener_arg_add_listener(&bar_set_listeners, (void (*)(const void *))listener);
}

void bar_add_on_get_listener(bar_on_get_listener listener)
{
	listener_add_listener(&bar_get_listeners, listener);
}
