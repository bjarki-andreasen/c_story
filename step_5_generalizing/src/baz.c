...
LISTENER_ARG_LISTENERS_DEFINE(baz_set_listeners, BAZ_N_LISTENERS);
...
{
	listener_arg_add_listener(&baz_set_listeners, (void (*)(const void *))listener);
}

void baz_add_on_get_listener(baz_on_get_listener listener)
{
	listener_add_listener(&baz_get_listeners, listener);
}
