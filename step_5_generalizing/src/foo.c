...
LISTENER_ARG_LISTENERS_DEFINE(foo_set_listeners, FOO_N_LISTENERS);
...
{
	listener_arg_add_listener(&foo_set_listeners, (void (*)(const void *))listener);
}

void foo_add_on_get_listener(foo_on_get_listener listener)
{
	listener_add_listener(&foo_get_listeners, listener);
}
